...
public class SizeConsoleCommand implements ConsoleCommand {

    private static final String COMMAND_NAME = "size";
    private final Cache<Long, Value> cache;

    public SizeConsoleCommand(Cache<Long, Value> cache) {
        this.cache = cache;
    }
    ...
    public boolean execute(TextUI console, Iterator<String> args) throws IllegalParametersException {
        try {
            ...
            console.println("Size of " + v + " is " + v.size() + " bytes");
        } catch (NumberFormatException e) {
            throw new IllegalParametersException("Expected usage: size <key>\nValue for key has to be a number. In example\nsize 10");
